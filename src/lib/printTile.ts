...
function wrapInPage(content: string, paper: { width: number; height: number }, pageNum: number, margin: number): string {
  return `<svg xmlns="http://www.w3.org/2000/svg"
    width="${paper.width}mm" height="${paper.height}mm"
    viewBox="0 0 ${paper.width} ${paper.height}">
    <g transform="translate(${margin} ${margin})">${content}</g>
    <text x="${paper.width - margin}" y="${paper.height - 3}"
      text-anchor="end" font-size="3" font-family="Arial,sans-serif">Page ${pageNum}</text>
  </svg>`
}

function calibrationSquare(margin: number): string {
  return `<rect x="${margin}" y="${margin}" width="10" height="10"
    fill="none" stroke="black" stroke-width="0.5"/>
  <text x="${margin}" y="${margin + 15}" font-size="3" font-family="Arial,sans-serif">10mm / 0.39in</text>`
}
